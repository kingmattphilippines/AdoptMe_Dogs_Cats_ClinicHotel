...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <div class="w3-container">
    <form action="admin_edit_pet_func.php" method="POST" enctype="multipart/form-data">
        <input type="hidden" class="form-control" name="ID" value="<?=$row['ID'];?>">
        <div class="form-group">
            <label for="petname">Pet Name</label>
            <input type="text" class="form-control" placeholder="eg. Brownie" name="PET_NAME" value="<?=$row['PET_NAME'];?>">
        </div>
        <div class="form-group">
            <label for="description">Description</label>
            <textarea class="form-control" placeholder="eg. Sample description" name="PET_DESC"><?=$row['PET_DESC'];?></textarea>
        </div>
        <div class="form-group">
            <label for="display">Display Image</label>
            <input type="file" name="PET_DISPLAY" value="<?=$row['PET_DISPLAY'];?>">
        </div>
        <div class="form-group">
            <label for="display">Display Image #1</label>
            <input type="file" name="PET_D[]" value="<?=$row['PET_D1'];?>">
        </div>
        <div class="form-group">
            <label for="display">Display Image #2</label>
            <input type="file" name="PET_D[]" value="<?=$row['PET_D2'];?>">
        </div>
        <div class="form-group">
            <label for="display">Display Image #3</label>
            <input type="file" name="PET_D[]" value="<?=$row['PET_D3'];?>">
        </div>
        <div class="form-group">
            <label for="display">Display Image #4</label>
            <input type="file" name="PET_D[]" value="<?=$row['PET_D4'];?>">
        </div>
        <br>
        <button type="submit" class="w3-button w3-green" name="update" id="update">Update</button>
    </form>
  </div>
  <!-- End page content -->
</div>
